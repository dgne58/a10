---
title: "Rate Limiting"
source: "https://linkerd.io/2-edge/reference/rate-limiting/"
author:
published:
created: 2026-04-15
...
tags:
  - "clippings"
---
Linkerd’s rate limiting functionality is configured via `HTTPLocalRateLimitPolicy` resources, which should point to a [Server](https://linkerd.io/2-edge/reference/authorization-policy/#server) reference. Note that a `Server` can only be referred by a single `HTTPLocalRateLimitPolicy`.

> [!-info] -info
> #### Note
> 
> `Server` ’s default `accessPolicy` config is `deny`. This means that if you don’t have [AuthorizationPolicies](https://linkerd.io/2-edge/reference/authorization-policy/) pointing to a Server, it will deny traffic by default. If you want to set up rate limit policies for a Server without being forced to also declare authorization policies, make sure to set `accessPolicy` to a permissive value like `all-unauthenticated`.

## HTTPLocalRateLimitPolicy Spec

| field | value |
| --- | --- |
| `targetRef` | A reference to the [Server](https://linkerd.io/2-edge/reference/authorization-policy/#server) this policy applies to. |
...
| `overrides` | An array of [overrides](https://linkerd.io/2-edge/reference/rate-limiting/#overrides) for traffic from specific client. |
