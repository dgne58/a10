---
title: "conn.log — Book of Zeek (8.1.1)"
source: "https://docs.zeek.org/en/current/logs/conn.html"
author:
published:
created: 2026-04-13
description:
...
The `orig_pkts` and `resp_pkts` fields report the number of IP packets transferred in the respective directions. The `orig_ip_bytes` and `resp_ip_bytes` indicate the total IP packet-level byte counts, respectively.
...
The numeric `ip_proto` field reports the [IP protocol number](https://www.iana.org/assignments/protocol-numbers/protocol-numbers.xhtml) of the connection. It relates to the `proto` field, but while the former represents a [`transport_proto`](https://docs.zeek.org/en/current/scripts/base/init-bare.zeek.html#type-transport_proto "transport_proto type") value that exclusively covers *transport* protocols Zeek knows how to parse (and ties into Zeek’s [`port`](https://docs.zeek.org/en/current/script-reference/types.html#type-port "port type") type), the `ip_proto` field is always present, including for non-transport IP packet flows such as IGMP or OSPF. For example, an OSPF flow might look as follows:
...
You can adapt this feature in several ways. Load the [policy/protocols/conn/ip-proto-name-logging.zeek](https://docs.zeek.org/en/current/scripts/policy/protocols/conn/ip-proto-name-logging.zeek.html) policy script to add an `ip_proto_name` column with a string version of the `ip_proto` value. Also, you may disable the whole feature by loading the [policy/protocols/conn/disable-unknown-ip-proto-support.zeek](https://docs.zeek.org/en/current/scripts/policy/protocols/conn/disable-unknown-ip-proto-support.zeek.html) script, returning `conn.log` to its pre-7.1 state. Zeek’s [logging framework](https://docs.zeek.org/en/current/frameworks/logging.html#framework-logging) supports additional customizations.
...
There are other fields which may appear in the `conn.log`, depending on the protocol being summarized. For details on the meaning of those fields, see [`Conn::Info`](https://docs.zeek.org/en/current/scripts/base/protocols/conn/main.zeek.html#type-Conn::Info "Conn::Info type").
