...
tags:
  - "clippings"
---
Linkerd’s [egress functionality](https://linkerd.io/2-edge/features/egress/) allows you to monitor and control traffic that leaves the cluster. This behavior is controlled by creating `EgressNetwork` resources, which describe the properties of traffic that leaves a cluster and provide a way to apply policies to it, using Gateway API primitives.
...
| field | value |
| --- | --- |
| `networks` | A set of [network specifications](https://linkerd.io/2-edge/reference/egress-network/#networks) that describe the address space that this `EgressNetwork` captures |
| `trafficPolicy` | the default [traffic policy](https://linkerd.io/2-edge/reference/egress-network/#trafficpolicy) for this resource. |
