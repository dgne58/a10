...
- Sidecar and perimeter proxies work as [Policy Enforcement Points](https://csrc.nist.gov/glossary/term/policy_enforcement_point) (PEPs) to secure communication between clients and servers.
...
To use `ClusterTrustBundle` in Istio, you must enable it by setting a flag during installation. Here’s how:
...
2. Add this to your istio configuration
	```yaml
	values:
	  pilot:
	    env:
...
- Suffix match: a string with an ending `"*"`. For example, `"test.abc.*"` matches `"test.abc.com"`, `"test.abc.com.cn"`, `"test.abc.org"`, etc.
- Prefix match: a string with a starting `"*"`. For example, `"*.abc.com"` matches `"eng.abc.com"`, `"test.eng.abc.com"`, etc.
...
- Try out the security policy by following the [authentication](https://istio.io/latest/docs/tasks/security/authentication/) and [authorization](https://istio.io/latest/docs/tasks/security/authorization/) tasks.
- Learn some security [policy examples](https://istio.io/latest/docs/ops/configuration/security/security-policy-examples/) that could be used to improve security in your mesh.
- Read [common problems](https://istio.io/latest/docs/ops/common-problems/security-issues/) to better troubleshoot security policy issues when something goes wrong.